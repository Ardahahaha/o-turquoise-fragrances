...
function DisplayCard({
  ...
  return (
    <div
      className={cn(
        "relative flex h-[120px] w-[260px] select-none flex-col justify-between rounded-[22px] border border-white/70 bg-white px-4 py-3.5 shadow-[0_10px_40px_-12px_rgba(0,0,0,0.12),0_4px_12px_-4px_rgba(0,0,0,0.06)] backdrop-blur-md transition-transform duration-500 ease-out hover:-translate-y-0.5",
        className,
      )}
    >
      <div className="flex items-center gap-2">
        <span className={cn("relative inline-flex rounded-full bg-turquoise/10 p-1", iconClassName)}>
          {icon}
        </span>
        <p className={cn("text-sm font-semibold leading-none", titleClassName)}>{title}</p>
      </div>
...
export function DisplayCards({ cards }: { cards?: DisplayCardProps[] }) {
  ...
  const positions = [
    ...
    },
    {
      ...
      z: "z-20",
      transform: "translate(0px, -40px) rotate(2deg)",
  ...
  return (
    <div className="relative mx-auto h-[250px] w-[340px] sm:h-[260px] sm:w-[360px]">
